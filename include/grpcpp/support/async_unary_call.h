...
class ClientAsyncResponseReader final : public ClientAsyncResponseReaderInterface<R> {
  public:
    ...
    void Finish(R* msg, Status* status, void* tag) override {
        (void)msg;
        (void)tag;
        *status = Status(StatusCode::UNIMPLEMENTED, "async CQ API not supported by grpc-lite");
    }
};
...
class ClientAsyncResponseReaderHelper {
  public:
    ...
    static ClientAsyncResponseReader<Response>* Create(
        ChannelInterface* channel, CompletionQueue* cq, const internal::RpcMethod& method,
