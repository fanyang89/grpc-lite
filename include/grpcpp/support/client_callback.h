...
void CallbackUnaryCall(
    ChannelInterface* channel, const RpcMethod& method, ClientContext* context,
    const Request* request, Response* response, std::function<void(Status)> on_completion
) {
    (void)channel;
    (void)method;
    (void)context;
    (void)request;
    (void)response;
    on_completion(Status(StatusCode::UNIMPLEMENTED, "callback API not supported by grpc-lite"));
}
...
class ClientCallbackUnaryFactory {
  public:
    ...
    static void Create(
        ChannelInterface* channel, const RpcMethod& method, ClientContext* context,
        const void* request, void* response, ClientUnaryReactor* reactor
    ) {
        (void)channel;
        (void)method;
        (void)context;
