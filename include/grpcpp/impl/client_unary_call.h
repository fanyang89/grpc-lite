...
template <
    class InputMessage, class OutputMessage, class BaseInputMessage = InputMessage,
    class BaseOutputMessage = OutputMessage>
Status BlockingUnaryCall(
    ChannelInterface* channel, const RpcMethod& method, grpc::ClientContext* context,
    ...
    Status status = channel->CallUnary(method.name(), context, request_bytes, &response_bytes);
    if (!status.ok()) {
        return status;
    }
